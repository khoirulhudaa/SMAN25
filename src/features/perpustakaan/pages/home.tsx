...
const HeroSection = () => {
  const scrollToLibrary = () => {
    document.getElementById("perpustakaan-content")?.scrollIntoView({ behavior: "smooth" });
  };

  return (
    <section className="relative h-[78vh] flex items-center justify-center overflow-hidden">
      {/* Background – Perpustakaan representatif */}
      <div
        className="absolute inset-0 bg-cover bg-center bg-no-repeat"
        style={{
          backgroundImage: `url('/kgr2.jpg')`,
      ...
      <div className="relative z-[1] text-left md:text-center text-white px-6 max-w-5xl -mt-10 mx-auto">
        ...
        >
          ...
          <p className="text-md md:text-xl mb-12 max-w-3xl mx-auto opacity-95 drop-shadow-lg">
            Akses koleksi buku digital, referensi, dan materi pembelajaran yang ada di SMAN 25 Jakarta kapan saja secara mudah.
          </p>
          <motion.button
            initial={{ opacity: 0, y: 40 }}
            animate={{ opacity: 1, y: 0 }}
...
const handleInputChange = (e) => {
  ...
  const totalReads = useMemo(
    () => apiBooks.reduce((s, b) => s + getReadCount(b.id), 0),
    [apiBooks]
  );
